    ...
    const fetchData = async () => {
      try {
        ...
        const data = await pb.collection('therapist_profile').getFullList({
          expand: 'therapistId', // 👈 this assumes therapistId is the relation field
        });
      ...
      {isLoading ? (
        ...
        <div className="flex flex-wrap justify-center gap-10 w-full">
          {therapists.map((therapist) => (
            <div
              key={therapist.id}
              className="bg-[#fffbf3] rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-[500px] w-[320px] m-2"
            >
              {/* Card Header */}
